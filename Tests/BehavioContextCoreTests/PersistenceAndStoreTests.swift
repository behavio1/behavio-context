...
final class PersistenceAndStoreTests: XCTestCase {
    ...
    func testFailedFinalizationSurfacesPreservedRecordingNotice() async {
        let recoveryURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("behaviocontext-recovery-\(UUID().uuidString)", isDirectory: true)
...
private func temporaryRecordingURL() -> URL {
    FileManager.default.temporaryDirectory.appendingPathComponent("behaviocontext-\(UUID().uuidString).mp4")
}

private func temporaryRecordingsDirectory() -> URL {
    FileManager.default.temporaryDirectory.appendingPathComponent(
        "behaviocontext-recorder-\(UUID().uuidString)",
        isDirectory: true
    )
}
...
private func makeVideoSampleBuffer(
    ...
    guard pixelStatus == kCVReturnSuccess, let pixelBuffer else {
        throw NSError(domain: "BehavioContextTests", code: Int(pixelStatus))
    }
    ...
    guard formatStatus == noErr, let formatDescription else {
        throw NSError(domain: "BehavioContextTests", code: Int(formatStatus))
    }
    ...
    guard sampleStatus == noErr, let sampleBuffer else {
        throw NSError(domain: "BehavioContextTests", code: Int(sampleStatus))
    }
    return sampleBuffer
}
...
private func makeWhiteVideoSampleBuffer(
    ...
    guard let pixelBuffer = sampleBuffer.imageBuffer else {
        throw NSError(domain: "BehavioContextTests", code: -1)
    }
    CVPixelBufferLockBaseAddress(pixelBuffer, [])
    defer { CVPixelBufferUnlockBaseAddress(pixelBuffer, []) }
    guard let baseAddress = CVPixelBufferGetBaseAddress(pixelBuffer) else {
        throw NSError(domain: "BehavioContextTests", code: -2)
    }
